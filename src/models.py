...
DEFAULT_MODEL_PATHS = {
    ...
    'detr-panoptic': MODELS_DIR / 'detr_panoptic_model'  # Renamed from 'dino-seg'
}

MODEL_URLS = {
    "yolov8n-seg.pt": "https://github.com/ultralytics/assets/releases/download/v0.0.0/yolov8n-seg.pt",
    "detr-panoptic": "facebook/detr-resnet-50-panoptic"  # Corrected key to match model type
...
# --- DETR Panoptic Wrapper ---
class DETRPanopticWrapper:
    """Wrapper for DETR (Detection Transformer) with panoptic segmentation using HuggingFace Transformers."""
    
    def __init__(self, model_path=DEFAULT_MODEL_PATHS['detr-panoptic']):
        """
        Initializes the DETR Panoptic model wrapper.
        
        Args:
            model_path (str or Path): Path or name of the DETR Panoptic model from HuggingFace.
        """
        print("Initializing DETRPanopticWrapper")
        ...
        # Store COCO class names
        self.class_names = COCO_CLASSES
        self.confidence_threshold = 0.45  # Default threshold for detections

        try:
            # Import necessary libraries here to avoid dependency issues if the model is not used
            from transformers import DetrForPanopticSegmentation, DetrForObjectDetection, AutoFeatureExtractor
            import huggingface_hub
            
            print(f"Loading model at path: {model_path}")
            
            # Always use the Hugging Face model ID directly - ignore local paths to avoid the backslash issues
            hf_model_id = MODEL_URLS.get("detr-panoptic")  # Use the correct key
            
            print(f"Using HuggingFace model ID: {hf_model_id}")
            
            # Create local directory if it doesn't exist
            local_dir = Path(model_path) if isinstance(model_path, str) else model_path
            local_dir.mkdir(parents=True, exist_ok=True)
            
            try:
                # Load the panoptic segmentation model from Hugging Face
                print(f"Loading panoptic segmentation model: {hf_model_id}")
                self.feature_extractor = AutoFeatureExtractor.from_pretrained(hf_model_id)
                self.model = DetrForPanopticSegmentation.from_pretrained(hf_model_id)
                self.has_segmentation = True
                
                # Save model locally for future use
                try:
                    print(f"Saving model to local path: {local_dir}")
                    self.feature_extractor.save_pretrained(str(local_dir))
                    self.model.save_pretrained(str(local_dir))
                    print(f"Model successfully saved to: {local_dir}")
                except Exception as save_err:
                    print(f"Note: Could not save model locally: {save_err}")
            except Exception as he:
                print(f"Error loading panoptic model: {he}")
                print("Trying with basic object detection model...")
                
                try:
                    # Fall back to standard DETR model which is public
                    alternate_model = "facebook/detr-resnet-50"
                    print(f"Loading alternate detection model: {alternate_model}")
                    self.feature_extractor = AutoFeatureExtractor.from_pretrained(alternate_model)
                    self.model = DetrForObjectDetection.from_pretrained(alternate_model)
                    self.has_segmentation = False
                    
                    # Try to save the model locally
                    try:
                        print(f"Saving alternate model to local path: {local_dir}")
                        self.feature_extractor.save_pretrained(str(local_dir))
                        self.model.save_pretrained(str(local_dir))
                        print(f"Alternate model successfully saved to: {local_dir}")
                    except Exception as save_err:
                        print(f"Note: Could not save alternate model locally: {save_err}")
            ...
            # Move model to the correct device
            self.model.to(self.device)
            print("DINO-Seg model loaded successfully")
            
        except ImportError as ie:
            print(f"Error importing required libraries for DINO-Seg: {ie}")
            print("Please install required dependencies with: pip install transformers timm huggingface_hub")
            self.model = None
        except Exception as e:
            print(f"Error loading DINO-Seg model: {e}")
            import traceback
            traceback.print_exc()
            self.model = None
    ...
    def predict(self, frame: np.ndarray):
        ...
        if self.model is None or self.feature_extractor is None:
            print("Warning: DINO-Seg model not loaded. Returning empty results.")
            annotated_frame = frame.copy()
            cv2.putText(annotated_frame, "DINO-Seg Model Not Loaded", (50, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            return [], [], annotated_frame
        ...
        try:
            ...
            # Get original image dimensions for scaling the bounding boxes
            img_h, img_w = frame.shape[:2]
            
            # Process output predictions
            detections = []
            segmentations = []  # For segmentation masks
            
            # Create a copy of the frame for drawing
            annotated_frame = frame.copy()
            
            # Check if we have the panoptic segmentation model or just detection
            if hasattr(self, 'has_segmentation') and self.has_segmentation:
                # Process panoptic segmentation output
                # Extract predictions for panoptic segmentation
                probas = outputs.logits.softmax(-1)[0, :, :-1]  # Remove the "no-object" class
                ...
                # Create a colorful segmentation overlay
                overlay = np.zeros_like(frame)
                
                # Get the segmentation result - this contains both stuff (background) and things (objects)
                if hasattr(outputs, 'pred_masks') and outputs.pred_masks is not None:
                    # Get the masks and process them
                    ...
                    # Process each detection with its mask
                    for i, (box, prob, mask) in enumerate(zip(boxes_to_keep, probas_to_keep, masks)):
                        ...
                        x1 = int(xmin.item() * img_w)
                        y1 = int(ymin.item() * img_h)
                        x2 = int(xmax.item() * img_w)
                        y2 = int(ymax.item() * img_h)
                        
                        # Skip small or invalid boxes
                        if x2 <= x1 or y2 <= y1 or (x2-x1)*(y2-y1) < 20:
                            continue
                        
                        # Get class name - add 1 to match COCO class index convention
                        adjusted_class_id = class_id + 1
                        class_name = self.class_names[adjusted_class_id] if adjusted_class_id < len(self.class_names) else f"Class_{class_id}"
                        ...
                        # Process mask - convert to numpy and resize to frame size
                        mask_np = mask.sigmoid().cpu().numpy() > 0.5
                        mask_resized = cv2.resize(mask_np.astype(np.uint8), (img_w, img_h))
                        segmentations.append(mask_resized * 255)  # Add to segmentations list
                    ...
                    # Combine the frame with the overlay
                    cv2.addWeighted(annotated_frame, 1.0, overlay, 0.5, 0, annotated_frame)
                    
                else:
                    # Handle case where there are no masks (shouldn't happen with panoptic model)
                    print("Warning: No segmentation masks found in panoptic model output")
                    # Fall back to detection-only processing
                    for i, (box, prob) in enumerate(zip(boxes_to_keep, probas_to_keep)):
                        # Find class with highest probability
                        class_id = prob.argmax().item()
                        score = prob[class_id].item()
                ...
                # Process regular object detection output (non-panoptic model)
                probas = outputs.logits.softmax(-1)[0, :, :-1]  # Remove the "no-object" class
                keep = probas.max(-1).values > self.confidence_threshold
            ...
            # Add detailed timing report to console
            print(f"DINO-Seg timing: Total={total_time:.3f}s (FPS: {fps:.1f}) | " 
                  f"Preprocess={preprocess_time:.3f}s | Inference={inference_time:.3f}s | "
                  f"Postprocess={postprocess_time:.3f}s | Detections={len(detections)} | Masks={len(segmentations)}")
            ...
            print(f"Error during DINO-Seg prediction: {e}")
            import traceback
            traceback.print_exc()
            annotated_frame = frame.copy()
            cv2.putText(annotated_frame, "DINO-Seg Prediction Error", (50, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            return [], [], annotated_frame
    ...
    def _initialize_model(self):
        ...
        try:
            if self.model_type == 'mask-rcnn':
                ...
                return DETRPanopticWrapper(self.model_path)
            # Keep dino-seg for backward compatibility
            elif self.model_type == 'dino-seg':
                print("Note: 'dino-seg' model type is deprecated, please use 'detr-panoptic' instead")
                return DETRPanopticWrapper(self.model_path)
            else:
